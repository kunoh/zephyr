...
{
    int ret = 0;
    ret = sensor_sample_fetch_chan(battery_dev, (sensor_channel)SENSOR_CHAN_BATTERY_GENERAL);

    if (ret != 0) {
        logger_.err("Could not trigger general sampling.");
    }
...
{
    int ret = 0;
    ret = sensor_sample_fetch_chan(battery_dev, (sensor_channel)SENSOR_CHAN_BATTERY_CHARGING);

    if (ret != 0) {
        logger_.err("Could not trigger charging sampling.");
    }
...
int BatteryNh2054qe34::GetGeneralData(BatteryGeneralData *data)
{
    ...
    ret =
        sensor_channel_get(battery_dev, (sensor_channel)SENSOR_CHAN_BATTERY_GENERAL, bat_gen_data);
    if (ret != 0) {
        logger_.err("Failed to get some battery general properties.");
    }
...
int BatteryNh2054qe34::GetChargingData(BatteryChargingData *data)
{
    ...
    ret =
        sensor_channel_get(battery_dev, (sensor_channel)SENSOR_CHAN_BATTERY_CHARGING, bat_chg_data);
    if (ret != 0) {
        logger_.err("Failed to get some battery charging properties.");
    }
...
int BatteryNh2054qe34::GetChargingCurrent(int32_t *charging_current)
{
    ...
    ret = sensor_channel_get(battery_dev, SENSOR_CHAN_GAUGE_DESIRED_CHARGING_CURRENT,
...
int BatteryNh2054qe34::GetChargingVoltage(int32_t *charging_volt)
{
    ...
    ret =
        sensor_channel_get(battery_dev, (sensor_channel)SENSOR_CHAN_GAUGE_DESIRED_CHARGING_VOLTAGE,
