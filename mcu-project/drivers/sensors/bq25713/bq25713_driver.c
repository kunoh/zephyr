...
static int bq25713_init(const struct device* dev, const struct bq25713_config* charger_config, const struct i2c_dt_spec charger_i2c_spec)
{
    int ret = 0;
...
static int bq25713_sample_fetch(const struct device* dev, enum sensor_channel chan)
{
    ...
    if(!i2c_is_ready_dt(&charger_i2c_spec))
    {
        return EBUSY;
    }
...
static int bq25713_attr_set(const struct device* dev, enum sensor_channel chan, enum sensor_attribute attr, const struct sensor_value* val)
{
    ...
    switch(attr)
    {
        case SENSOR_ATTR_CONFIGURATION:
            switch (chan)
            {
                case SENSOR_CHAN_INIT_CONFIG:
                    ret = bq25713_init(dev, charger_config, charger_i2c_spec);
